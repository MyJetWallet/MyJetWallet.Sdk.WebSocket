...
namespace MyJetWallet.Sdk.WebSocket
{
    public abstract class WebsocketEngine : IDisposable
    {
        ...
        public static int InitBufferSize { get; set; } = 128;

        private Task _runTask;

        private readonly CancellationTokenSource _token = new CancellationTokenSource();

        public WebsocketEngine(string name, string url, int pingIntervalMSec, int silenceDisconnectIntervalMSec,
            ILogger logger)
        {
            _name = name;
            _url = url;
        ...
        protected abstract void InitHeaders(ClientWebSocket clientWebSocket);
        ...
        public void Start()
        {
            ...
            _logger.LogInformation("Web socket {name} is started", _name);
        }
        ...
        public void Stop()
        {
            ...
            _logger.LogInformation("Web socket {name} is stopped", _name);
        }
        ...
        public async Task Run()
        {
            while (!_token.IsCancellationRequested)
            {
                try
                {
                    ...
                    InitHeaders(_clientWebSocket);
                    _logger.LogInformation("Web socket {name} try to connect ...", _name);
                    await _clientWebSocket.ConnectAsync(new Uri(_url), _token.Token);
                    _logger.LogInformation("Web socket {name} is connected", _name);

                    await Task.WhenAll(Receive(_clientWebSocket), Send(_clientWebSocket),
                        CheckActivity(_clientWebSocket));
                }
                ...
                finally
                {
                    ...
                    _logger.LogInformation("Web socket {name} is closed", _name);
                }
                ...
                {
                    _logger.LogError(ex, "Web socket {name} receive Exception from method OnDisconnect", _name);
                }
        ...
        private async Task CheckActivity(ClientWebSocket webSocket)
        {
            try
            {
                ...
                while (webSocket.State == WebSocketState.Open && !_token.IsCancellationRequested)
                {
                    ...
                    {
                        _logger.LogError(
                            "Web socket {name} was closed because do not receive messages from service side", _name);
            ...
            {
                _logger.LogError(ex, "Web socket {name} receive Exception on check activity");
            }
        }
        ...
        private async Task Send(ClientWebSocket webSocket)
        {
            try
            {
                ...
                _logger.LogInformation("Web socket {name} OnConnected is done", _name);
            ...
            {
                _logger.LogError(ex, "Web socket {name} receive Exception and closed", _name);
        ...
        private async Task Receive(ClientWebSocket webSocket)
        {
            try
            {
                ...
                {
                    var result =
                        await webSocket.ReceiveAsync(new ArraySegment<byte>(buffer, offset, buffer.Length - offset),
                            _token.Token);
                    if (result.MessageType == WebSocketMessageType.Close)
                    {
                        _logger.LogInformation("Web socket {name} receive close signal", _name);
                    ...
                    if (!result.EndOfMessage)
                    {
                        ...
                        _logger.LogInformation("Web socket {name} increase buffer size to {size}", _name,
                            buffer.Length);
